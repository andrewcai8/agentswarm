...
function registerTools(server: McpServer): void {
  server.tool(
    ...
    "Launch the AgentSwarm orchestrator to begin parallel autonomous coding. Resets the target repo and starts a fresh run.",
    {
      request: z
        ...
        .optional()
        .describe(
          `Build request (defaults to '${DEFAULT_REQUEST}')`,
        ),
    },
    async ({ request }) => {
  ...
  server.tool(
    "stop_swarm",
    "Gracefully stop the running AgentSwarm orchestrator",
    {},
    async () => {
  ...
  server.tool(
    ...
    async () => {
      ...
      if (latestMetrics) {
        ...
        lines.push(
          `Active Workers: ${m.activeWorkers}/${m.activeWorkers + m.pendingTasks}`,
        );
        ...
        lines.push(
          `Merge Rate: ${(m.mergeSuccessRate * 100).toFixed(1)}%`,
        );
        lines.push(
...
httpServer.listen(PORT, () => {
  console.log(`AgentSwarm MCP server running at http://localhost:${PORT}/mcp`);
  console.log();
  ...
  console.log();
  console.log(
    `  npx poke tunnel http://localhost:${PORT}/mcp --name "AgentSwarm"`,
  );
  console.log();
});
